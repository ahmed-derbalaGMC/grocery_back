...
router.get('/all/', function (req, res, next) {
  return models.Order.find({ $or: [{ name: req.query.name }, { quantity: req.query.quantity }] })
    .then(Product => {
      if (Product == null) {
...
router.get('/mine/', function (req, res, next) {
  return models.Order.find({ UserId: req.cu._id})
    .then(Product => {
      if (Product == null) {
        log().verbose({ message: `no products were found`, route: req.originalUrl, ip: req._remoteAddress, userAgent: req.headers["user-agent"] });
        return res.status(404).send('no products were found')
...
router.post('/buy/', function (req, res, next) {  
  return models.Order({ UserId: req.cu._id, ProductId: req.query.ProductId }).save((err, myObj) => {
    if (err) {
      return res.status(errorHandler(err).status).send(errorHandler(err).error)
    }
    log().verbose({ message: 'bought' })
...
router.delete('/destroy/:id', function (req, res, next) {
  return models.Order.findByIdAndRemove(req.params.id)
    .then(myObj => {
      if (myObj == null) {
        log().verbose({ message: `${myModel}._id=${req.params.id} not found`, route: req.originalUrl, ip: req._remoteAddress, userAgent: req.headers["user-agent"] });
        return res.status(404).send(`${myModel}._id=${req.params.id} not found`)
      }
      return res.status(200).send(`${myModel}._id=${req.params.id} destroyed`)
    })
    .catch(err => {
      log().error({ message: err, route: req.originalUrl, ip: req._remoteAddress, userAgent: req.headers["user-agent"] });
      console.log(colors.bgRed(err));
